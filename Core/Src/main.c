...
int main(void)
{
  ...
  configure_gpio(HAVECONTROLLER_PORT, HAVECONTROLLER_PIN, LL_GPIO_MODE_OUTPUT, LL_GPIO_PULL_NO);

  for (int i = 0; i < 10; i++) {
    write_gpio(HAVECONTROLLER_PORT, HAVECONTROLLER_PIN, true);
    delay(100);
    write_gpio(HAVECONTROLLER_PORT, HAVECONTROLLER_PIN, false);
    delay(100);
  }
  ...
  while (1) {
    ...
    write_gpio(HAVECONTROLLER_PORT, HAVECONTROLLER_PIN, haveController);
    ...
    if (!haveController) {
      if (psxc_begin()) {
        ...
        psxc_read();    // Make sure the protocol is up to date
      ...
      if (!psxc_read()) {
        ...
        psxc_dumpButtons(psxc_getButtonWord());

        int8_t lx = 0, ly = 0;
        psxc_leftAnalogMoved(&lx, &ly);
        if (lx != slx || ly != sly) {
          psxc_dumpAnalog("Left", lx, ly);
          slx = lx;
          sly = ly;
        }

        int8_t rx = 0, ry = 0;
        psxc_rightAnalogMoved(&rx, &ry);
        if (rx != srx || ry != sry) {
          psxc_dumpAnalog("Right", rx, ry);
          srx = rx;
          sry = ry;
        }
